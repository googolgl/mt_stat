...
mt_stat.timeout = minetest.settings:get("mt_stat.timeout") or 10
mt_stat.db = minetest.settings:get("mt_stat.db") or "minetest"
mt_stat.receive = minetest.settings:get("mt_stat.receive") or 30
...
function mt_stat.send_metrics()
	...
	minetest.after(mt_stat.timeout, mt_stat.send_metrics)
end

function mt_stat.request_http(url,pd)
	local req = {url = mt_stat.url..url, post_data = pd, extra_headers = {mt_stat.header}, timeout = mt_stat.interval}
